...
//
// Sprint 1, Day 3: a real catalog-style list endpoint — paginated so it
// stays fast regardless of how many patients exist, with optional filters
// applied conditionally (only when actually supplied), and projected to a
// DTO rather than exposing the Patient entity directly.
[HttpGet]
    ...
    [FromQuery] string sortDirection = "asc")
{
    ...
    {
        ("age", "desc") => query.OrderBy(p => p.DateOfBirth),        // oldest DOB = youngest last -> oldest first when ascending DOB... see note below
    ...
    // Skip/Take applied after filtering and sorting, and the projection to
    // PatientResponse happens as part of the same query — EF Core
    // translates this to SQL that only selects the columns actually
    // needed, rather than loading full entities and mapping in memory.
